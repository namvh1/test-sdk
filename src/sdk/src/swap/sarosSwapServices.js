...
export const getSwapExactOutSaros = async (
  connection,
  fromCoinMint,
  toCoinMint,
  amount,
  ...
  if (
    ...
    const invariant = convertAmountInputToken * convertAmountOutputToken;
    ...
    const amountInWithSlippage = tokenInAmountWithFee / (1 - newSlippage / 100);

    const beforePrice = convertAmountOutputToken / convertAmountInputToken;
    const afterPrice = newPoolTokenOutAmount / newPoolTokenInAmount;
    const priceImpact = Math.abs(
      ((parseFloat(beforePrice) - parseFloat(afterPrice)) /
        parseFloat(beforePrice)) *
        100
    ...
    const invariant = convertAmountInputToken * convertAmountOutputToken;
    const newPoolTokenInAmount = convertAmountInputToken - newAmount;
    const newPoolTokenOutAmount = invariant / newPoolTokenInAmount;
    const tokenInAmount = newPoolTokenOutAmount - convertAmountOutputToken;
    let tokenInAmountWithFee =
      (tokenInAmount *
        (tradeFeeDenominator.toNumber() + tradeFeeNumerator.toNumber())) /
      tradeFeeDenominator;
    ...
    const amountInWithSlippage = tokenInAmountWithFee / (1 - newSlippage / 100);
    const beforePrice = convertAmountInputToken / convertAmountOutputToken;
    const afterPrice = newPoolTokenInAmount / newPoolTokenOutAmount;
    const priceImpact = Math.abs(
      ((parseFloat(afterPrice) - parseFloat(beforePrice)) /
        parseFloat(afterPrice)) *
        100
    );
    return {
      amountIn: tokenInAmountWithFee,
